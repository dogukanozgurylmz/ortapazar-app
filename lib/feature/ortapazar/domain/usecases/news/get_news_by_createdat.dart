import 'package:ortapazar/core/errors/failures.dart';
import 'package:dartz/dartz.dart';
import 'package:ortapazar/feature/ortapazar/domain/entities/news_entity.dart';

import '../../../../../core/usecase/usecase.dart';
import '../../repositories/news_repository.dart';

class GetNewsByCreatedAtUseCase
    extends UseCase<List<NewsEntity>, GetNewsByCreatedAtParams> {
  final NewsRepository newsRepository;

  GetNewsByCreatedAtUseCase({
    required this.newsRepository,
  });

  @override
  Future<Either<Failure, List<NewsEntity>>> call(
      GetNewsByCreatedAtParams params) async {
    final result = await newsRepository.getNewsByCreatedAt(
      params.collectionId,
      params.query,
    );
    final either = result.fold((l) => l, (r) => r);
    if (either is List<NewsEntity> && either.isNotEmpty) {
      return Right(either);
    }
    return Left(ServerFailure());
  }
}

class GetNewsByCreatedAtParams {
  final String collectionId;
  final String query;

  GetNewsByCreatedAtParams({
    required this.collectionId,
    required this.query,
  });
}
